...
HandInfo dealer_hand = { 0 };
HandInfo player_hand = { 0 };
...
static void Deal()
{
    ...
    for (int i = 0; i < 5; ++i)
    {
        player_hand.cards[i] = deck_ids.at(deal_index++); 
        dealer_hand.cards[i] = deck_ids.at(deal_index++);
    }

    player_hand.high_card = 0;
    player_hand.rank = 0;
    player_hand.jokers.reset();
...
static void RankHand(HandInfo* hand)
{
    ...
    if (straight_found && flush_found)                                                // royal flush
    {
        if ((hand->cards[4] % 13 == 0))  hand->rank = 30;
        else if ((hand->cards[4] % 13 == 12) && (offset == 1)) hand->rank = 30;
        else if ((hand->cards[4] % 13 == 11) && (offset == 2)) hand->rank = 30;
        else if ((hand->cards[4] % 13 == 10) && (offset == 3)) hand->rank = 30;
        else hand->rank = 29;
    }
    ...
    */
    ...
    while (!quit)
    {
        ...
        if (ch == 'd') {
            ...
            for (int id : card_ids)
            {
                player_hand.cards[id] = deck_ids[deal_index++];
