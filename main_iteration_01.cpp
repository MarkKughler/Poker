...
static void Deal()
{
    ...
    for (int i = 0; i < 5; ++i)
    {
        player.cards[i] = deck_ids.at(deal_index++);
        dealer.cards[i] = deck_ids.at(deal_index++);
    }

    player.highest_card_index = 0;
    player.rank = 0;
    dealer.highest_card_index = 0;
    dealer.rank = 0;
}
...
int main()
{
    ...
    */
    ...
    std::cout << "Dealer: "; DisplayHand(&dealer);
    std::cout << "Player: "; DisplayHand(&player);
    ...
    while (!quit)
    {
        ...
        if (ch == 'd') {
            ...
            for (int id : card_ids)
            {
                player.cards[id] = deck_ids[deal_index++];
